...
**New discovery, not yet built**: `NewPlayerDialog.tsx`'s `Previous`/`Next` navigation buttons are a genuinely different button family — plain text-only links (no background, no uppercase, `text-sm text-stone-500`), not a match for `primary` or `secondary`. Left untouched (out of scope for this adoption) and confirmed as a real, evidenced candidate (2 real instances in this one file) for a future `tertiary` intent or separate text-link button style — not yet designed or built. `destructive` intent and `IconButton.tsx` remain future work — not yet started. `StatTile` not yet started. Planned next: design/build a `tertiary` intent (or separate text-link button component) for the `Previous`/`Next` pattern found in `NewPlayerDialog.tsx`, then `destructive` intent, then `IconButton.tsx`, then `StatTile`.
