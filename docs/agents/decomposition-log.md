...
**Status**: Layers 1–3 **complete and verified**.
...
**Confirmed still-open gap, correctly not fixed in this layer**: `useEncounterPresetLoader.ts` still doesn't pass real `legendaryActionsMax`/`legendaryResistancesMax` values to `addEncounterCombatantDB` — freshly-added NPCs will still show `0` remaining legendary actions/resistances until this is wired, even though the reload path (this layer) is now fully correct. Scheduled for Layer 4, alongside `useCombatantMutations.ts`.

Next: Layer 4 (`useCombatantMutations.ts` — the actual persistence-writing fix for legendary/recharge state — plus `useEncounterPresetLoader.ts`'s initial-add wiring).
