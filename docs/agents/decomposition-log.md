...
**Migration status: 10 of 12 complete.** Done: `LongRestDialog.tsx`, `CasterAttributionDialog.tsx`, `ShortRestDialog.tsx`, `NewEncounterDialog.tsx`, `NewNpcDialog.tsx`, `ShortcutCheatSheet.tsx`, `ReferenceDetailDialog.tsx`, `EncounterLogModal.tsx`, `NewPlayerDialog.tsx`, `LevelUpDialog.tsx`. Each migration was verified independently via TypeScript build check plus its relevant test batch before moving to the next.
...
**Next steps (2 remaining)**: `AddCombatantDialog.tsx` (renamed from `CombatSidebar.tsx` — see note above; not yet migrated to `DialogShell`) and `Soundboard.tsx` (has two separate overlay-like UI pieces — a right-click context menu and an assignment dialog — only the assignment dialog is a `DialogShell` migration candidate). `CommandPalette.tsx` remains excluded from migration entirely (see above).
