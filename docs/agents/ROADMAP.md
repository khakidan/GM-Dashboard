...
**Status**: `Badge`, `ToggleBadge`, and `PipTracker` are all done (see `CHANGELOG.md`). Next: pick one of the remaining candidates above and run its own small, single-topic verification pass before designing — same discipline that's worked well for all three components so far, not a repeat of the original large multi-part audit.