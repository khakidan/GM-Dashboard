...
**`Badge`, `ToggleBadge`, `PipTracker`, `Accordion`, `Callout`, and `SearchInput` are all complete** — see `CHANGELOG.md` for the full write-ups. What follows is everything else confirmed during this round's audit that hasn't been designed or built yet.
...
**Status**: `Badge`, `ToggleBadge`, `PipTracker`, `Accordion`, `Callout`, and `SearchInput` are all done (see `CHANGELOG.md`). Next: pick one of the remaining candidates above and run its own small, single-topic verification pass before designing — same discipline that's worked well for all six components so far, not a repeat of the original large multi-part audit.