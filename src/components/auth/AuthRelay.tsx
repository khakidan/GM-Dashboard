...
export function AuthRelay() {
  const { signInWithRedirect, signInWithToken } = useGoogleAuth();
  ...
  if (!token) {
    return (
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-6 text-center">
        <div className="bg-[#ffffff] p-8 rounded-3xl shadow-2xl max-w-lg w-full border border-[#2563eb]/30">
          <div className="mb-6 flex justify-center">
            <div className="w-16 h-16 bg-[#2563eb]/10 rounded-full flex items-center justify-center">
               <RefreshCcw className="w-8 h-8 text-[#2563eb]" />
            </div>
          </div>
          
          <h1 className="text-2xl font-bold text-[#0f172a] mb-2 font-serif uppercase tracking-tight">Sync Authentication</h1>
          <p className="text-sm text-[#8d8db9] mb-8 font-sans">Connect your Google Sheets campaign to the web hub.</p>
          
          <div className="space-y-6">
            <div className="p-6 bg-white border-2 border-[#2563eb]/20 rounded-2xl text-left bg-gradient-to-br from-white to-[#ffffff]">
              <div className="flex items-center gap-3 mb-4">
                 <div className="w-6 h-6 bg-[#2563eb] text-white rounded-full flex items-center justify-center text-xs font-bold shrink-0">!</div>
              ...
              <button 
                ...
                className="w-full bg-[#2563eb] text-white px-8 py-5 rounded-xl font-bold uppercase tracking-widest text-[11px] shadow-xl hover:bg-[#567eff] transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
              >
          ...
          <div className="mt-8 pt-6 border-t border-black/5">
            <button 
              ...
              className="text-[10px] uppercase font-bold text-[#8d8db9] hover:underline opacity-60 flex items-center gap-1 mx-auto"
            >
            ...
            {showTroubleshooting && (
              <div className="mt-6 text-left p-6 bg-[#0f172a] rounded-2xl text-white/90 text-[11px] space-y-5 shadow-inner">
                <div>
                   <p className="font-bold text-[#2563eb] mb-1 uppercase tracking-widest text-[9px]">1. Authorized Redirect URI</p>
                ...
                <div>
                   <p className="font-bold text-[#2563eb] mb-1 uppercase tracking-widest text-[9px]">2. Missing Refresh Token?</p>
                ...
                <div>
                  <p className="font-bold text-[#2563eb] mb-1 uppercase tracking-widest text-[9px]">3. 400: redirect_uri_mismatch</p>
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-6">
      <div className="bg-[#ffffff] p-8 rounded-3xl shadow-2xl max-w-xl w-full border-2 border-[#2563eb]">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-12 h-12 bg-[#2563eb] rounded-full flex items-center justify-center text-white shadow-inner">
            <Check className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-[#0f172a] font-serif uppercase tracking-tight">Login Successful!</h1>
            <p className="text-xs text-[#8d8db9] uppercase tracking-widest font-bold opacity-60">Here are your persistence codes</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {refreshToken ? (
            ...
            <div className="p-6 bg-red-50 border-2 border-red-200 rounded-2xl shadow-sm">
               ...
               <div className="p-4 bg-white/50 rounded-xl border border-red-100 flex flex-col gap-3">
                  <p className="text-[9px] text-[#8d8db9] uppercase font-bold">To fix this permanently:</p>
                  <ol className="text-[10px] text-[#8d8db9] space-y-2 list-decimal pl-4">
          ...
          <div className="p-6 bg-[#e2e8f0] border-2 border-[#e2e8f0] rounded-2xl opacity-80">
            <p className="text-[11px] text-[#0f172a] font-bold uppercase tracking-wider mb-2 opacity-70">
            ...
            <div className="relative group">
              <textarea 
                readOnly
                value={token || ''}
                className="w-full h-24 bg-white border border-[#e2e8f0] rounded-xl p-4 text-[11px] font-mono text-[#8d8db9] break-all focus:ring-2 focus:ring-[#2563eb]/20 outline-none resize-none shadow-inner"
              />
              <button 
                onClick={() => handleCopy(token || '')}
                className="absolute top-3 right-3 bg-[#0f172a] text-white px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 hover:bg-black transition-all shadow-lg active:scale-95"
              >
                {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                Copy Temporary Code
              </button>
          ...
          <div className="flex gap-3 text-left bg-[#2563eb]/10 p-4 rounded-xl border border-[#2563eb]/20">
             <div className="w-6 h-6 bg-[#2563eb] text-white rounded-full flex items-center justify-center text-xs font-bold shrink-0">?</div>
             <p className="text-[11px] text-[#8d8db9] leading-snug">
          ...
          <div className="mt-8 pt-6 border-t border-black/5 flex flex-col gap-3">
            ...
            <a href="/#/" className="text-[10px] uppercase font-bold text-[#8d8db9]/60 hover:underline">Return to Dashboard</a>
          </div>
        </div>
      </div>
    </div>
