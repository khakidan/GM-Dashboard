    ...
    <div className="bg-white rounded-2xl shadow-sm border border-[#e5e1d8] overflow-hidden flex-1 flex flex-col w-full">
      ...
      <div className="bg-[#fdfaf5] border-b border-[#e5e1d8] p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-[#2c2c26]">Party Roster</h1>
            <p className="text-sm text-[#5a5a40] mt-0.5">Manage your players characters. Any edits here instantly sync back to Google Sheets.</p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              onClick={() => setIsLongRestOpen(true)}
              disabled={isResting || state.characters.length === 0}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[#e5e1d8] hover:border-[#c5b358] text-[#5a5a40] hover:text-[#2c2c26] text-xs font-bold uppercase tracking-widest rounded-xl transition-all shadow-sm disabled:opacity-50"
              id="party-long-rest-btn"
            >
              {isResting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Coffee className="w-3.5 h-3.5 text-[#c5b358]" />}
              Long Rest
            </button>
            <button
            ...
            >
              <Coffee className="w-3.5 h-3.5 text-[#c5b358]" />
              Short Rest
            </button>
            <button
              onClick={() => setIsNewPlayerDialogOpen(true)}
              disabled={isAddingPlayer}
              className="flex items-center gap-1.5 px-4 py-1.5 bg-[#c5b358] hover:bg-[#d4c47a] focus:ring-1 focus:ring-[#c5b358] outline-none text-[#2c2c26] text-xs font-bold uppercase tracking-widest rounded-xl transition-all shadow-md hover:shadow-lg disabled:opacity-50"
              id="add-player-btn"
            >
