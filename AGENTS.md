...
- `services/` audit pass — done. Findings: 10 files, zero upward layer violations (no services/ file imports from hooks/ or components/), spreadsheet ID resolution correctly centralized in sheetsService.ts, no raw fetch() calls outside sheetsService.ts/googleAuth.ts (the latter's 3 calls are correctly scoped to internal /api/auth/* endpoints, not the Sheets API). One minor naming collision was found: queueWrite was exported by both src/services/dbOperations/shared.ts (a wrapper that resolves the spreadsheet ID before calling the core function) and src/services/writeQueue.ts (the core queuing implementation). RESOLVED — renamed the wrapper in src/services/dbOperations/shared.ts from queueWrite to queueWriteResolved. Updated call sites in npcs.ts and characters.ts accordingly. Verified: TypeScript clean, services batch 34/34 passing.
...
- **Component Built**: src/components/ui/IrvSection.tsx has been created and verified (TypeScript clean, exit code 0 confirmed twice independently). Confirmed via direct file comparison that it correctly generalizes both CharacterIRVSection.tsx and NpcIRVSection.tsx's identical logic, parameterizing labels/placeholders/gap as props with no baked-in defaults except gap (defaults to 'gap-4').
- **Adoption Phase 1 Completed**: CombatTab.tsx and NpcCombatTab.tsx migrated to use the shared IrvSection component. Required one design addition beyond the original spec: IrvSection.tsx gained an optional `compact?: boolean` prop, since CombatTab.tsx requires it. Process note: the initial migration introduced a real, caught-and-fixed visual regression — CombatTab.tsx originally never passed a `placeholder` prop (relying on IrvMultiSelect's ES6 default parameter fallback text 'Search and add...'), but the migrated code initially passed explicit empty strings, which suppress the default (ES6 defaults only trigger on `undefined`, not empty string). Fixed by passing the actual default text explicitly. Verified: TypeScript clean, PartyTab batch 46/46 and NpcLibraryTab batch 13/13 passing. Remaining adoption sites: CharacterIRVSection.tsx and NpcIRVSection.tsx (not yet migrated).
- **Final Adoption & Complete Migration**: Final two adoption sites migrated: CharacterCardExpanded.tsx and NpcCard.tsx now call IrvSection directly with their original hardcoded labels/placeholders/gap values preserved exactly (Character: 'Resistances'/'Immunities'/'Vulnerabilities', 'e.g. fire'/'e.g. poison'/'e.g. cold', gap-4; NPC: 'Resists'/'Immune'/'Vuln', 'None'/'None'/'None', gap-3). CharacterIRVSection.tsx and NpcIRVSection.tsx have been deleted entirely — confirmed via direct file-not-found check and a zero-result grep for any remaining references anywhere in the codebase. Full 12-batch verification: 692/692 passing, TypeScript clean. All 4 confirmed adoption sites (CombatTab.tsx, NpcCombatTab.tsx, CharacterCardExpanded.tsx, NpcCard.tsx) now use the single shared IrvSection.tsx component.
...
Separate, unrelated finding: CommandPalette.tsx's dead import (effectiveMaxHp, applyLongRestToConditions from '../lib/conditions') has been removed. Verified: TypeScript clean, CommandPalette test batch 13/13 passing.
...
Verification note: a clean, individually-verified 12-batch run was completed at the start of today's session, confirming 692/692 passing with real per-file output — the baseline is now trustworthy. (An earlier check on this specific file had produced degraded, non-verifiable output; this has since been superseded by a clean verification.)
