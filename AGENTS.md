...
- `hooks/` audit pass — PARTIALLY verified. One confirmed finding: `useLevelUpAutomation.ts` imports the `PoolEdit` type from `LevelUpResourcePools.tsx` (a component file) — a minor upward layer violation. Fix: move the `PoolEdit` interface to `src/types.ts`. All other claims from this pass (total hook file count, largest files, absence of name collisions, hooks lacking React APIs) are NOT verified with raw command output and should not be treated as confirmed until independently re-checked.
